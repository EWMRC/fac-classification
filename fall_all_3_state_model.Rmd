...
male_spring_init_dates <- readRDS("spring_male_3_state_model.rds") %>%
  filter(step_state == 2) %>%
  group_by(ID) %>%
...
female_spring_init_dates <- readRDS("spring_female_3_state_model.rds") %>%
  filter(step_state == 2) %>%
  group_by(ID) %>%
  group_modify(.f = function(x, ...){head(x, n = 1)}) %>%
  ungroup() %>%
...
saveRDS(amwo_hmm, file = "fall_all_3_state_model.rds")
```
