...
Extract all locations from 2018, 2019, 2020, and 2021 from August 1st through January 4th
```{r include=FALSE}
...
amwo_2019 <- getMovebankData(study = "American Woodcock Migration Ecology in Eastern North America", login = readRDS("movebank_credentials.rds"), timestamp_start = "20190801000000000", timestamp_end = "20200225000000000", removeDuplicatedTimestamps=TRUE) %>% as.data.frame()
...
nested_birds_2.5$data <- nested_birds_2 %>% 
  ...
  future_map(.f = function(x){
    ...
    for(i in 1:(nrow(observed_locs)-1)){
      ...
      if(point_dist > 30200 & ((point_2$rownum - point_1$rownum) > 1)){ 
...
filter(!(ID == "PA-2021-38-2021" & time > ymd_hms("2022-01-29 12:00:00"))) %>% 
  filter(!(ID == "NJ-2018-03-2018" & time == ymd_hms("2019-01-18 02:59:12"))) %>% 
  filter(!(ID == "PA-2018-01-2018" & julian_day == 298)) %>% #ranging movement
  filter(!(ID == "ME-2018-08-2018" & julian_day %in% c(301, 302, 303)))
  # filter(!(ID == "PA-2019-15-2019" & julian_day == 290)) %>%  #ranging movement pt1
  # filter(!(ID == "PA-2019-15-2019" & julian_day == 291)) #ranging movement pt2
  #Note: this is going to either have to be manually categorized or categorized as "not designated" in the final model
```
