...
amwo_hmm <- amwo_hmm %>%
  ...
  dplyr::select(ID, animal_name, locType, x, y, time, date, julian_day) %>% 
  as.data.frame()
```
...
dist <- list(step="gamma", angle="wrpcauchy", log_mean_dist_7 = "norm") #, lat_diff_abs = "gamma" , pts_10mi_log = "norm"
```
...
fixPar <- list(beta=matrix(c(NA, -100, # pre-migration to transitions, can't go pre- to post
                     ...
                     NA, -100,
                     -100, NA,
                     -100, -100), nrow = 3, byrow = TRUE)) #once entered post-migration, can't leave
```

These serve as the starting parameters for the HMM to optimize. Do these need to change for fall?
```{r}
