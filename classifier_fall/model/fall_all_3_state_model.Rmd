...
```{r}
amwo_spring <- as.data.frame(rbind(amwo_2017, amwo_2018, amwo_2019, amwo_2020))
...
```{r}
amwo_spring_males <- amwo_spring_males %>%
  mutate(julian_day = yday(ymd(date)- days_subtract)) #subtracting 5 to make sure that January julian days don't wrap around
```
...
```{r include = FALSE}
# note: need to ensure that capture sheet updates regularly. Include update code somewhere in the project?
...
male_spring_init_dates <- readRDS(here::here("classifier_spring", "spring_male_3_state_model.rds")) %>%
  filter(step_state == 2) %>%
  group_by(ID) %>%
  group_modify(.f = function(x, ...){head(x, n = 1)}) %>%
  ungroup() %>%
  mutate(ID = paste0(animal_name, "-", (as.numeric(year) - 1))) %>% #Create IDs that will match the IDs of the previous fall, if the transmitter had been deployed then. If not, this will create invalid ids that will "come out in the wash"
  transmute(ID = ID, init_time = time)

female_spring_init_dates <- readRDS(here::here("classifier_spring", "spring_female_3_state_model.rds")) %>%
  filter(step_state == 2) %>%
  group_by(ID) %>%
  group_modify(.f = function(x, ...){head(x, n = 1)}) %>%
  ungroup() %>%
...
# remove a late summer movement which throws off the hmm for SC-2020-13
amwo_hmm <- amwo_hmm %>%
  filter(!(ID == "SC-2020-13-2020" & date == "2020-08-02")) %>%
  filter(!(ID == "SC-2020-13-2020" & date == "2020-08-06")) %>%
  filter(!(ID == "RI-2020-31-2020" & date == "2020-11-21"))
```
...
Do these need to change for fall?
```{r}
Par0_m1 <- list(step=c(0.485, 211.927, 0.485, #mean in km
                ...
                julian_day = c(269, 300, 330, #mean Oct 1, Nov 1, Dec 1, all minus 5
                        10, 10, 10))
...
Ruleset: Post-migration is all points up until the bird comes within 16.1km of its final location for the first time, assuming that its final location is in state 3
```{r}
amwo_hmm <- map(unique(amwo_hmm$ID), function(ind){
  last_loc <- amwo_hmm %>%
    filter(ID == ind) %>%
  ...
  amwo_ind <- amwo_hmm %>%
    ...
    st_drop_geometry() %>%
    mutate(within_dist_post = condition_distance_km_post < 16.1) #16.1
  ...
  if(lowest_postmig_row< -1000){
    cat(crayon::magenta("no points within 16.1 kilometers of the last point"))
  }
...
saveRDS(amwo_hmm, file = here::here("classifier_fall", "fall_all_3_state_model.rds"))
```
