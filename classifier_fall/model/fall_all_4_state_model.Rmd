...
known_state_key <- amwo_hmm %>%
  ...
  #mutate(known = ifelse(ID == "RI-2020-42-2020" & julian_day == 331, 2, known)) %>% #fix a bird which goes into 4 too early. This should be superflous after line 67.
  ...
  mutate(known = ifelse(ID == "NY-2019-11-2019" & julian_day == 309, 4, known)) %>% #fix a bird which settles after 2/25
  mutate(known = ifelse(ID == "RI-2020-37-2020" & julian_day == 365, 4, known)) %>% #fix a bird which settles after 2/25. Temp change to make non-mig classification work, not listed in manuscript.
  mutate(known = ifelse(ID == "RI-2020-40-2020" & julian_day == 365, 4, known)) %>% #fix a bird which settles after 2/25. Temp change to make non-mig classification work, not listed in manuscript.
  mutate(known = ifelse(ID == "QUE-2021-25-2021" & julian_day == 350, 4, known)) %>% #fix a bird which settles after 2/25. Temp change to make non-mig classification work, not listed in manuscript.
...
fit_and_predict_base <- function(.x){
  set.seed(8)
...
# 11 post mig changes, 6 pre-mig changes
```
...
```{r}
amwo_hmm_final <- amwo_hmm_final %>%
  filter(#ID != "PA-2019-13-2019", #already excluded
    #ID != "RI-2019-18-2019", #already excluded
    ...
    #ID != "FL-2021-01-2021", #already excluded
