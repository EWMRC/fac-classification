...
fit_and_predict <- function(.x){
  ...
  .x$point_state <- map(1:nrow(.x),  function(i){
    ...
    } else if(.x[i,"step_state"] == 2 & .x[i-1,"step_state"] == 1 & .x[i,"ID"] == .x[i-1,"ID"]){ #is the current step state mig and the previous step pre-mig, and this is not step 1? Return pre-mig
      return(1) 
