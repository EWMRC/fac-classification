...
male_spring_init_dates <- readRDS(here::here("classifier_spring", "spring_male_4_state_model.rds")) %>%
  filter(step_state == 2) %>%
  group_by(ID) %>%
...
dist <- list(step="gamma", angle="wrpcauchy", y = "norm", julian_day = "norm", log_mean_dist_7 = "norm", residence_time = "norm") #, lat_diff_abs = "gamma" , pts_10mi_log = "norm"
```
...
Par0_m1 <- list(step=c(0.485, 211.927, 0.485, 0.485, #mean in km
                ...
                y = c(41.58981, 36.79636, 36.79636, 34.32931,
                      4.433215, 4.035114, 2.398324, 2.398324),
                julian_day = c(231.1194, 269.1586, 269.1586, 313.0377, #priors pulled from the model w/o julian day
                        27.13328, 27.70735, 25.30101, 25.30101),
...
Ruleset: Post-migration is all points up until the bird comes within 16.1km of its final location for the first time, assuming that its final location is in state 4
```{r}
amwo_hmm_final <- map(unique(amwo_hmm_final$ID), function(ind){
...
saveRDS(results_initial$model, file = here("classifier_spring", "fall_all_4_state_parameters.rds"))
```
