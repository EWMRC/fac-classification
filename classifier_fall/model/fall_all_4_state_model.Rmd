...
#         sum()
# 
#       if(total_dist_moved < 30.2){
#         start_of_last_stopover <- iter_df[1:(last_3_row-1),] %>%
#           filter(step_state == 2) %>% #the last stopover occurs right after the last mig step
#           pull(row_num) %>%
#           max(.) + 1
# 
...
#       mutate(., row_num = 1:nrow(.))
# 
#     if(any(iter_df$point_state == 3) & any(iter_df$point_state == 1)){
#       first_3_row <- iter_df %>%
#         filter(point_state == 3) %>%
#         pull(row_num) %>%
#         min()
