...
Identify lazarus ids, so that we can put them into a known post-mig state for their final location (not enough data to converge on its own)
```{r}
lazarus_ids <- amwo_hmm %>% #identify lazarus schedules
  group_by(ID) %>% 
  group_map(.f = function(x, y){
    early_flag <- any(x$julian_day < (yday(ymd("2020/9/1") - days_subtract)))
    late_flag <- any(x$julian_day > (yday(ymd("2020/12/15") - days_subtract)))
    between_flag <- any(x$julian_day > (yday(ymd("2020/9/1") - days_subtract)) & x$julian_day < (yday(ymd("2020/12/15") - days_subtract)))
    
    if(early_flag & late_flag & !between_flag){
      y %>% #unique(x$ID)
        pull(ID) %>% 
        as.character() %>% 
        return()
...
lazarus_ids <- lazarus_ids[!is.na(lazarus_ids)] %>% 
  unlist()
```
...
known_state_key <- amwo_hmm %>%
  ...
  #mutate(known = ifelse(ID == "NY-2019-11-2019" & julian_day == 309, 4, known)) %>% #fix a bird which has a spring mig route starting after 2/25. This should be superflous after line 74.
  ...
  group_modify(.f = function(x, y){ #any lazarus id points end in a known final state
    max_rank <- x %>% 
      pull(ranks) %>% 
      max()
    if(pull(y, ID) %in% lazarus_ids){
      x <- x %>% 
        mutate(known = ifelse(ranks == max_rank, 4, known)) #set the state of the last row to settled
    }
    return(x)
