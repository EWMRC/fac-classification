...
---
Note: this is pretty broken, and still doesn't display the dates for dispersal movements
...
migratory_state_graph <- weekly_pt2 %>% 
  filter(primary_point_state %in% c("Foray loop", "Migratory (summer)")) %>% 
  ggplot(aes(x = standardized_date, y = pct, group = primary_point_state, col = primary_point_state)) +
  geom_hline(yintercept = c(2,4,6,8,10)/10, color = "gray90") +
  theme_bw() +
  scale_x_date(date_labels = "%b", # "%b %d"
  ...
  scale_y_continuous(breaks = c(0.02, 0.04, 0.06, 0.08, 0.1), limits = c(0, 0.1)) + #
  ...
  labs(col = "Migratory status",
       caption = "Y axis: Percent of all locations in a week that belong to a given migratory state")
...
migratory_state_graph <- migratory_state_graph + geom_line()
...
ggsave("secondary_state_graph.png")
```
