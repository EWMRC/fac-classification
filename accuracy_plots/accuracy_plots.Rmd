...
results_spring_female_null <- readRDS(here("classifier_spring", "accuracy", "null_female", "accuracy_results_female_null.rds"))

results_spring_male_full <- readRDS(here("classifier_spring", "accuracy", "full_male", "accuracy_results_male_full.rds"))

results_spring_male_null <- readRDS(here("classifier_spring", "accuracy", "null_male", "accuracy_results_male_null.rds"))

results_fall_full <- readRDS(here("classifier_fall", "accuracy", "full_acc_assessment", "accuracy_results_fall_full.rds"))

results_fall_null <- readRDS(here("classifier_fall", "accuracy", "null_acc_assessment", "accuracy_results_fall_null.rds"))
```

Manipulate the dataframes so there's one row per changed endstate
```{r}
acc_springmale <- bind_rows(results_spring_male_full, results_spring_male_null)

acc_springfemale <- bind_rows(results_spring_female_full, results_spring_female_null)

acc_fall <- bind_rows(results_fall_full, results_fall_null)

acc_springmale$nested_data <- acc_springmale %>% 
...
acc_springfemale$nested_data <- acc_springfemale %>% 
  dplyr::select(to_constrain, end_modified, end_true) %>% 
  pmap(.f = function(to_constrain, end_modified, end_true){
    target_df <- to_constrain %>% 
      rename(ID = id_iter)
...
acc_springfemale <- acc_springfemale %>% 
  transmute(cutoff_date = jul_day_cutoff, variable_to_test, nested_data) %>% 
  unnest(cols = nested_data) %>% 
  distinct() %>% 
  identify_errors_varimp() %>% 
...
acc_combined <- bind_rows(acc_springmale, acc_springfemale, acc_fall) %>% 
  mutate(season = factor(season, levels = c("Fall", "Spring (male)", "Spring (female)"), ordered = TRUE)) %>% 
  rename(model_type = variable_to_test) %>% 
  mutate(model_type = recode(model_type, "null" = "Base", "full" = "Full")) %>% 
  mutate(model_type = factor(model_type, levels = c("Full", "Base"), ordered = TRUE))
```
...
dummy <- tibble(standardized_date = ymd(c("2020-10-01", "2020-10-01", "2020-03-01", "2020-03-01", "2020-03-01", "2020-03-01", "2021-02-05", "2021-02-05", "2020-07-01", "2020-07-01", "2020-07-01", "2020-07-01")), percent_error = 0, season = c("Fall", "Fall", "Spring Male", "Spring Male", "Spring Female", "Spring Female", "Fall", "Fall", "Spring Male", "Spring Male", "Spring Female", "Spring Female"), model_type = c("Full", "Base", "Full", "Base", "Full", "Base", "Full", "Base", "Full", "Base", "Full", "Base"), test = "Type I") %>% 
  bind_rows(tibble(standardized_date = ymd(c("2020-10-01", "2020-10-01", "2020-03-01", "2020-03-01", "2020-03-01", "2020-03-01", "2021-02-05", "2021-02-05", "2020-07-01", "2020-07-01", "2020-07-01", "2020-07-01")), percent_error = 0, season = c("Fall", "Fall", "Spring Male", "Spring Male", "Spring Female", "Spring Female", "Fall", "Fall", "Spring Male", "Spring Male", "Spring Female", "Spring Female"), model_type = c("Full", "Base", "Full", "Base", "Full", "Base", "Full", "Base", "Full", "Base", "Full", "Base"), test = "Type II")) %>% 
  mutate(season = recode(season, 
                         "Spring Male" = "Spring (male)",
                         "Spring Female" = "Spring (female)")) %>% 
  mutate(season = factor(season, levels = c("Fall", "Spring (male)", "Spring (female)"), ordered = TRUE)) %>% 
  mutate(model_type = factor(model_type, levels = c("Full", "Base"), ordered = TRUE))
```

```{r}
ggplot(data = acc_combined, mapping = aes(x = standardized_date, y = percent_error, col = model_type)) +
  ...
  labs(y = "Percent error", col = "Model") +
  scale_color_manual(values = c("#07646DFF", "#D4419EFF")) +
  theme(legend.position="bottom") +
  facet_grid(cols = vars(season), rows = vars(test),
             scales = "free_x") +
  geom_blank(data=dummy)

ggsave("accuracy_plot_6_26_24.png", width = 7, height = 5)
```

Getting medians and ranges
```{r}
acc_combined %>% 
  group_by(model_type, test, season) %>% 
  summarise(median = median(percent_error)*100, 
            range_min = min(percent_error)*100,
            range_max = max(percent_error)*100) %>% 
  View(title = "summary")
```
