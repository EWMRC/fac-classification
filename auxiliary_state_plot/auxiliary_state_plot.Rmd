...
#   project("EPSG:5070")
basemap_ras <- rast("C:/Users/liama/OneDrive - Kansas State University/MaineAnalysis/Data/basemaps/esri_lightgrey/esri_lightgrey_36_48_4326.tif") %>%
  project("EPSG:5070")
# basemap_ras <- rast("D:/OneDrive - Kansas State University/MaineAnalysis/Data/basemaps/esri_lightgrey/esri_lightgrey_36_48_4326.tif") %>% 
#   project("EPSG:5070")
```
...
template_map <- ggplot(range_shp) +
  ...
  geom_sf(aes(fill = season), alpha = 0.5, fill = c("#A65141FF", "#80A0C7FF", "#394165FF")) +
...
template_map_legend <- ggplot(range_shp) +
  theme_void() +
  geom_spatraster_rgb(data = basemap_ras, maxcell = 5e+07) + #5e+05
...
colors_dispersal <- c("#005C31", "#4C005C", "#0075DC", "#C20088")
names(colors_dispersal) <- dispersal_sorted

# Revised map
dispersal_map <- template_map +
  geom_sf(data = dispersal_line,
          mapping = aes(fill = animal_name, color = animal_name),
          linewidth = 0.5,
          arrow = arrow(length = unit(0.2, "cm"), type = "closed"))+#,
          #colour = c("#005C31", "#4C005C", "#0075DC", "#C20088"),
          #fill = c("#005C31", "#4C005C", "#0075DC", "#C20088")) +
  #geom_sf(data = dispersal_sf, size = 1, color = "black") +
  #geom_sf(data = dispersal_sf, size = 0.8, color = c("#F0A0FF", "#4C005C", "#0075DC", "#C20088")) +
  coord_sf(xlim = c(1637348, 2215039), ylim = c(2192378, 2770069), expand = FALSE) +
  theme(legend.position = "bottom",
        legend.margin=margin(t = 10),
        plot.margin = margin(b = 10)) +
  ggtitle("Directional movement") +
  scale_color_manual(values = colors_dispersal) +
  scale_fill_manual(values = colors_dispersal) +
...
# colors_foray <- c("black", "#0075DC", "#993F00","#4A6990FF", "#4C005C", "#C20088", "#005C31", "#2BCE48", "#FFCC99", "#808080","#94FFB5", "#8F7C00", "#9DCC00")
colors_foray <- paletteer::paletteer_d("awtools::bpalette") %>% as.character()

colors_foray <- c(colors_foray, "#C62828FF", "#F44336FF") #repeats colors to bring the palette up to the required number of categories

names(colors_foray) <- foray_sorted

# Revised map
foray_map <- template_map +
  geom_sf(data = foray_line, 
          mapping = aes(color = animal_name),
          linewidth = 0.5) + 
  geom_sf(data = foray_sf, size = 1, color = "black") +
  geom_sf(data = foray_sf, mapping = aes(color = animal_name, 
                                         fill = animal_name), 
          size = 0.8) +
  #coord_sf(xlim = c(-100, -60), ylim = c(25, 55), expand = FALSE) +
  coord_sf(xlim = c(-408613, 2763993), ylim = c(226720.6, 3589842), expand = FALSE) +
  theme(legend.position = "none") +
  ggtitle("Foray loop") +
  scale_color_manual(values = colors_foray) +
  scale_fill_manual(values = colors_foray)
...
colors_summer <- c("#4C005C", "#0075DC", "#993F00")
names(colors_summer) <- summer_sorted

# Revised map
summer_map <- template_map +
  geom_sf(data = summer_line,
          mapping = aes(color = animal_name,
                        fill = animal_name),
          linewidth = 0.5,
          arrow = arrow(length = unit(0.2, "cm"), type = "closed")) +
  #geom_sf(data = summer_sf, size = 1, color = "black") +
  #geom_sf(data = summer_sf, size = 0.8, color = c("#4C005C", "#0075DC", "#993F00")) +
  #coord_sf(xlim = c(-100, -60), ylim = c(25, 55), expand = FALSE) +
  coord_sf(xlim = c(-408613, 2763993), ylim = c(226720.6, 3589842), expand = FALSE) +
  theme(legend.position = "none") +
  ggtitle("Summer migration") +
  scale_color_manual(values = colors_summer) +
  scale_fill_manual(values = colors_summer)
...
```{r}
legend_desc <- get_legend(template_map_legend)

timelines_combined <- ggarrange(summer_timeline, dispersal_timeline, foray_timeline, nrow = 3, ncol = 1, heights = c(3+5, 4+5, 13+5))
...
ggsave(filename = here::here("auxiliary_state_plot", "auxiliary_state_plot_6_27_2024.png"), plot = plots_arranged, dpi = 300, bg = "white", height = 7.5, width = 7)
```
