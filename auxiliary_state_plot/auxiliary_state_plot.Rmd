...
dispersal_map <- template_map +
  geom_sf(data = dispersal_line, linewidth = 0.5, colour = c("blue", "pink", "green")) +
  geom_sf(data = dispersal_sf, size = 1, color = "black") +
  geom_sf(data = dispersal_sf, size = 0.8, color = c("blue", "pink", "green")) +
  coord_sf(xlim = c(-100, -60), ylim = c(25, 55), expand = FALSE) +
...
map_timeline_dispersal <- ggarrange(blank,
                                    dispersal_map, 
                                    dispersal_timeline, 
                                    ncol = 1, nrow = 3,
                                    heights = c(1,10,2),
...
foray_map <- template_map +
  geom_sf(data = foray_line, linewidth = 0.5, colour = c("blue", "pink", "green","blue", "pink", "green","blue", "pink", "green","red","orange", "green", "blue")) + 
  geom_sf(data = foray_sf, size = 1, color = "black") +
  geom_sf(data = foray_sf, size = 0.8, color = c("blue", "pink", "green","blue", "pink", "green","blue", "pink", "green","red","orange", "green", "blue")) +
  coord_sf(xlim = c(-100, -60), ylim = c(25, 55), expand = FALSE) +
  theme(legend.position = "none")
...
foray_timeline <- ggplot(data = foray_summary, mapping = aes(x = value, y = animal_name, group = animal_name, color = animal_name)) +
  ...
  theme(axis.title.y = element_blank(),
        ...
        axis.ticks.y=element_blank(),
        legend.position = "none") +
  scale_color_manual(values = c("blue", "pink", "green","blue", "pink", "green","blue", "pink", "green","red","orange", "green", "blue"))

map_timeline_foray <- ggarrange(blank,
                                foray_map, 
                                foray_timeline, 
                                ncol = 1, nrow = 3,
                                heights = c(1,10,5),
...
summer_map <- template_map +
  geom_sf(data = summer_line, linewidth = 0.5, colour = c("blue", "pink", "green")) + 
  geom_sf(data = summer_sf, size = 1, color = "black") +
  geom_sf(data = summer_sf, size = 0.8, color = c("blue", "pink", "green")) +
  coord_sf(xlim = c(-100, -60), ylim = c(25, 55), expand = FALSE) +
  theme(legend.position = "none")
...
map_timeline_summer <- ggarrange(blank,
                                 summer_map, 
                                 summer_timeline, 
                                 ncol = 1, nrow = 3,
                                 heights = c(1,10,2),
...
plots_left <- ggarrange(map_timeline_dispersal, map_timeline_summer, ncol = 1, nrow = 2, labels = c("Dispersal movements", "Summer migrations"))

plots_right <- ggarrange(map_timeline_foray, legend_desc, ncol = 1, nrow = 2, labels = c("Foray loops"), heights = c(1.62,1))

plots_arranged <- ggarrange(plots_left, plots_right, ncol = 2, nrow = 1)

# plots_arranged <- ggarrange(map_timeline_dispersal, 
#                             map_timeline_foray, 
#                             map_timeline_summer, 
#                             legend_desc, 
#                             ncol = 2, nrow = 2, 
...
ggsave(filename = here::here("auxiliary_state_plot", "auxiliary_state_plot_6.png"), plot = plots_arranged, dpi = 300, bg = "white", height = 7, width = 7)
```
