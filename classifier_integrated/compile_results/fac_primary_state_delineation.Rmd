...
Manually set a late settlement (after 6-30) and add in migratory movements with only two points (won't fit an HMM)
```{r}
...
migration_dates <- migration_dates %>% 
  bind_rows(tibble(animal_name = "WV-2024-58", time_initiation = ymd_hms("2025-02-01 16:00:00"), time_termination = NA, init_state = "Stationary", season = "Spring")) %>% 
  bind_rows(tibble(animal_name = "WV-2024-29", time_initiation = ymd_hms("2024-07-19 05:00:48"), time_termination = ymd_hms("2025-02-01 16:00:00"), init_state = "Stationary", season = "Fall")) %>% 
  bind_rows(tibble(animal_name = "WV-2024-31", time_initiation = ymd_hms("2024-07-19 05:00:48"), time_termination = ymd_hms("2025-02-01 16:00:00"), init_state = "Stationary", season = "Fall"))
```
...
capture_dates <- readxl::read_excel(here("capture_sheet.xlsx"), col_types = c("text", "text", "date", "numeric", "numeric", "text", "text", "text", "text", "text", "text", "text", "text", "numeric", "numeric", "numeric", "numeric", "numeric", "text", "text", "text", "text", "text", "text", "text", "text", "text", "text", "text", "text", "date", "numeric", "text", "date", "date", "text", "numeric", "text", "text", "text", "text", "text", "numeric")) %>% 
  mutate(Recap = if_else(Movebank_ID %in% c("RI-2018-01", "RI-2018-03", "RI-2018-04", "RI-2018-05", "RI-2018-06", "RI-2018-08", "RI-2018-09", "RI-2018-12", "RI-2018-13", "RI-2019-16", "RI-2019-17", "RI-2019-18", "RI-2019-19", "RI-2019-20", "RI-2019-23", "RI-2019-24", "RI-2019-25", "RI-2019-26", "RI-2019-28"), "No", Recap)) %>% ##### BUG: RI birds are not being interpreted correctly due to the Recap clause
  filter(Recap == "No" | is.na(Recap)) %>% 
  transmute(animal_name = Movebank_ID,
            capture_date = Date)

amwo_data <- amwo_data %>% 
  left_join(capture_dates) %>% 
  mutate(capture_date = if_else(animal_name == "VA-2021-95", mdy("3/1/2021"), capture_date)) #filling in a better estimated capture date for this individual
```
...
amwo_data$primary_step_state <- map2_chr(amwo_data$animal_name, amwo_data$time, function(t_id, t_date){ # iterates through individual woodcock locations
  ...
  if(nrow(tmd) == 0){
    ...
    winter_states <- c("AL", "FL", "GA", "LA", "NC", "SC", "NJ", "MD")
    summer_states <- c("ME", "NS", "NY", "ONT", "PA", "QUE", "VT", "WI", "WV")
    both_states <- c("VA", "RI")
    ...
    if(capture_state %in% winter_states) {
      ...
      return("Summer")
    } else if(capture_state %in% both_states & (tmp_nonmig_data$standard_date[1] > mdy("10/25/2020") | tmp_nonmig_data$standard_date[1] < mdy("2/25/2020"))) {
      return("Winter")
    } else if(capture_state %in% both_states & (tmp_nonmig_data$standard_date[1] < mdy("10/25/2020") & tmp_nonmig_data$standard_date[1] > mdy("2/25/2020"))) {
      return("Summer")
    } else {
...
amwo_data$primary_point_state <- map2_chr(amwo_data$animal_name, amwo_data$time, function(t_id, t_date){ # iterates through individual woodcock locations
  ...
  for(i in nrow(tmd):1){ #iterates through migratory bouts. This is a backwards classifier: it looks weird, but I swear it works
    if(is.na(tmd$time_termination[i]) & t_date > tmd$time_initiation[i]){ # if after initiation date, and the movement never ceased
      ...
      target <- if_else(tmd$season[i] == "Spring", "Migratory (spring)", "Migratory (fall)")
      certain <- 1
    } else if(t_date == tmd$time_initiation[i] & tmd$init_state[i] == "Stationary"){ # on the start date, and init state is stationary
      target <- if_else(tmd$season[i] == "Spring", "Winter", "Summer")
      certain <- 1
    } else if(t_date == tmd$time_initiation[i] & tmd$init_state[i] == "Migratory"){ # on the start date, and init state is migratory
      target <- if_else(tmd$season[i] == "Spring", "Migratory (spring)", "Migratory (fall)")
      certain <- 1
    } else if(certain == 0 & nrow(tmd) == i & if_else(is.na(tmd$time_termination[i]), FALSE, t_date >= tmd$time_termination[i])) { # if this is the final migratory movement (with an end date), and it occurred on/after the termination date
      target <- if_else(tmd$season[i] == "Spring", "Summer", "Winter")
...
source(here::here("classifier_integrated", "compile_results", "nonmigratory_designation.R"))
...
Run nesting script
```{r}
source(here::here("nests", "nest_location_assignment.R")) #disregard the unexpected match warning
```
