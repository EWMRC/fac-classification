...
```{r}
var_imp_springmale <- bind_rows(var_imp_springmale, results_spring_male_full, results_spring_male_null)

var_imp_springfemale <- bind_rows(var_imp_springfemale, results_spring_female_full, results_spring_female_null)

var_imp_fall <- bind_rows(var_imp_fall, results_fall_full, results_fall_null)
...
Summarise error by summming over weeks, and adjusting error by subtracting from the full model error

Actually, this is not interpretable as it stands. Running again using a mean error
```{r}
var_imp_summary <- var_imp %>% 
  group_by(season, variable_to_test, test) %>% 
  summarize(total_error = mean(percent_error)) %>% 
  ungroup()
...
var_imp_null_model <- var_imp_summary %>% 
  filter(variable_to_test == "null") %>% 
  dplyr::select(-variable_to_test) %>% 
  rename(null_model_error = total_error)

var_imp_summary <- var_imp_summary %>% 
  left_join(var_imp_full_model) %>% 
  left_join(var_imp_null_model) %>% 
  mutate(adjusted_error = full_model_error - total_error) %>% 
  mutate(full_null_error = full_model_error - null_model_error) %>% 
  mutate(proportion_error = adjusted_error*100/full_null_error) %>% 
  mutate(variable_to_test = recode(variable_to_test,
                                   "dist_start" = "Distance from \nstart",
                                   "breeding_abundance" = "Breeding \nrange",
                                   "log_mean_dist_7" = "Log(distance to \nnearest points)",
                                   "julian_day" = "Ordinal day",
                                   "y" = "Latitude",
                                   "residence_time" = "Residence time",
                                   "null" = "Base")) %>% 
  mutate(variable_to_test = factor(variable_to_test, levels = c("Distance from \nstart", "Breeding \nrange", "Log(distance to \nnearest points)", "Ordinal day", "Latitude", "Residence time", "Base"), ordered = TRUE))
...
var_imp_summary %>% 
  ...
  ylab("Reduction in accuracy from the full model") +
  labs(fill = "Error type") +
  scale_y_continuous(labels = scales::percent)

ggsave("variable_importance_3_15_24.png", width = 7.5, height = 4)
```

Calculating stats
```{r}
var_imp_summary %>% 
  filter(variable_to_test != "Base") %>% 
  pull(adjusted_error) %>% 
  median(.)*100 #-0.3149074
...
var_imp_summary %>% 
  filter(variable_to_test == "Residence time", test == "Type II") %>% 
  pull(adjusted_error) %>% 
  median(.)*100 #-17.16539
...
var_imp_summary %>% 
  filter(variable_to_test == "Base", test == "Type II") %>% 
  pull(adjusted_error) %>% 
  min(.)*100 #-42.55136
