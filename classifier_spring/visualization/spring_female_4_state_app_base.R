...
server <- function(input, output) {
  ...
  amwoData.sm <- readRDS(here::here("classifier_spring", "spring_female_4_state_model_base.rds"))
  ...
  selected_columns <- filter(amwoData.sm, ID == unique(amwoData.sm$ID)[1]) %>% 
    dplyr::select("ID", "point_state", "step_state", "date", "julian_day", "step", "angle", "locType",  "step_state", "unk_init_flag", "unk_term_flag") 
