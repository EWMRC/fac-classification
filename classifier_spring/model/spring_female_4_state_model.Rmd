...
starting_state_key <- starting_state_key %>% 
  ...
  mutate(estimate_starting_state = if_else(ID %in% c("NJ-2018-08-2019"
), true = FALSE, false = estimate_starting_state)) %>%
  mutate(estimate_starting_state = if_else(ID %in% c("WV-2025-85-2025"
), true = TRUE, false = estimate_starting_state)) %>% #this bird was still migrating when tagged
...
known_state_key <- amwo_hmm %>%
  ...
  mutate(known = ifelse(ID == "WI-2023-01-2023" & ranks == 1, 3, known)) %>%  # caught during mig
  mutate(known = ifelse(ID == "WV-2023-20-2023" & ranks == 1, 3, known)) %>%  # caught during mig
  mutate(known = ifelse(ID == "WV-2023-23-2023" & ranks == 1, 2, known)) %>%  # caught during mig
  mutate(r1p1s2 = ifelse(ID == "WV-2023-23-2023" & ranks == 1, FALSE, r1p1s2)) %>%  # exempt from r1s1p1 designation, as it was caught during migration
  mutate(known = ifelse(ID == "WV-2024-33-2024" & ranks == 1, 2, known)) %>% #caught during mig
...
amwo_hmm_final <- amwo_hmm_final %>%
  filter(ID != "NJ-2019-28-2020", # missing bird, exclude from paper
         ...
         ID != "RI-2021-52-2022",
         ID != "NS-2024-36-2025") #Fall migration does not terminate
```
