...
fit_and_predict <- function(.x){
  ...
  .x$point_state <- map(1:nrow(.x),  function(i){
    if(i == 1){
      return(1) #for the first row, return 1 #NOTE: this will change when I incorporate estimable starting states
    } else if(.x[i,"ID"] != .x[i-1,"ID"]){ #is this the first point for this ID? Return the starting state
      return(1) #NOTE: this will change when I incorporate estimable starting states
