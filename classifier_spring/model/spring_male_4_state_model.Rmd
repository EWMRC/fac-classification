...
dist <- list(step="gamma", angle="wrpcauchy", y = "norm", julian_day = "norm", log_mean_dist_7 = "norm", residence_time = "norm", dist_start = "norm") #, lat_diff_abs = "gamma" , pts_10mi_log = "norm"
```

fix step length parameters and state transitions
```{r}
fixPar <- list(beta=matrix(c(NA,	-1000,	-1000,	-1000,	-1000,	NA,	NA,	NA,	-1000,	NA,	-1000,	-1000,	-1000,	-1000,	-1000,	NA,	-1000,	-1000,	-1000,	NA), nrow = 1, byrow = TRUE), dist_start = c(NA, NA, 14.023368, 14.023368, 14.023368,
               NA, NA, 1, 1, 1)) #Estimate the dist_start of pre-mig and mig, but hold the dist_start of stopover, settling, and post-mig to be identical) #once entered post-migration, can't leave
```
...
Par0_m1 <- list(step=c(0.485, 211.927, 0.485, 1, 0.285, #mean in km
                ...
                dist_start = c(log(100), log(10000), log(10000), log(20000), log(20000),
                               log(100), log(10000), log(10000), log(10000), log(10000))) 
