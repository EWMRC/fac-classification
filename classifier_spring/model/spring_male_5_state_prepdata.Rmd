...
amwo_2025 <- getMovebankData(study = "American Woodcock Migration Ecology in Eastern North America", login = readRDS(here::here("movebank_credentials.rds")), timestamp_start = "20241221000000000", timestamp_end = "20250630000000000", removeDuplicatedTimestamps=TRUE) %>% as.data.frame() %>% 
  filter(timestamp > if_else(local_identifier == "LA-2024-45", ymd_hms("2024-12-21 00:00:00"), ymd_hms("2025-1-05 00:00:00"))) #last point before migration for this bird is in December
...
amwo_spring_males <- amwo_spring_males %>%
  ...
  mutate(year = if_else(ID == "LA-2024-45" & date == mdy("12/21/2024"), "2025", year)) %>%  # fixing a bug
...
```{r}
amwo_spring_males <- amwo_spring_males %>%
  mutate(julian_day = yday(ymd(date) + days(15))) # to deal with julian days that start before jan 1
```
...
# load(file=here("classifier_spring", "spring_male_data_temp.RData"))
```
