...
Load data
```{r}
load(file=here("classifier_spring", "spring_male_data_base.RData"))
```
...
starting_state_key <- starting_state_key %>% 
  ...
  mutate(estimate_starting_state = if_else(ID %in% c("NJ-2019-22-2020"), true = FALSE, false = estimate_starting_state)) %>% #Two birds were tagged in Dec, but didn't start taking points until much later, which causes the estimator to push them into mig as an initial states
...
fit_and_predict <- function(.x){
  set.seed(8)
  m1 <- fitHMM(data=.x, nbStates=5, dist=list(step="gamma", angle="wrpcauchy", step_500 = "bern"), 
               Par0 = list(step=c(0.293641627, 79.029506936, 0.157361645, 0.250739740, 0.086056474, #mean in km
...
# 1 individual had post-mig, 7 had pre-mig
```
...
saveRDS(amwo_hmm_final, file = here("classifier_spring", "spring_male_5_state_model_base.rds"))
saveRDS(results_initial$model, file = here("classifier_spring", "spring_male_5_state_parameters_base.rds"))
```
