...
```{r}
load(file=here("classifier_spring", "spring_female_data_base.RData"))
```
...
starting_state_key <- starting_state_key %>% 
  ...
  mutate(estimate_starting_state = if_else(ID %in% c("NJ-2018-09-2019", "NJ-2018-10-2019", "NJ-2018-13-2019", "NJ-2018-15-2019", "NJ-2019-23-2020", "VA-2019-36-2020", "VA-2020-67-2021", "VA-2021-92-2021"), true = FALSE, false = estimate_starting_state)) %>% #Two birds were tagged in Dec, but didn't start taking points until much later, which causes the estimator to push them into mig as an initial states
...
known_state_key <- amwo_hmm %>%
  ...
  mutate(known = ifelse(ID == "NY-2022-42-2022" & ranks == 1, 3, known)) %>%  #bird that migrated after nesting starts in stopover state
  mutate(known = ifelse(ID == "RI-2018-11-2019" & ranks == 94, 4, known)) #bird is recaptured and re-transmittered at the last mig location the following Oct, so we know that it is settled here
...
fit_and_predict_base <- function(.x){
  ...
  m1 <- fitHMM(data=.x, nbStates=4, dist=list(step="gamma", angle="wrpcauchy", step_500 = "bern"), 
               Par0 = list(step=c(0.260832696, 9.429288e+01, 0.323209095, 0.1581120, #mean in km
                                  0.434046082, 9.829083e+01, 0.565636024 , 0.2682378, # SD in km
                                  0.002864053, 1.549953e-09, 0.008935152, 0.0101826), #zeromass ie how many zeroes expected in the distribution
                           angle=c(0.001212793, -0.004443658, -0.004534878, 0.0003999368, #mean
                                   0.882238488, 0.911903062, 0.405040026, 0.7535390314),
...
# 6 post mig changes, 2 pre-mig changes
```
...
saveRDS(amwo_hmm_final, file = here::here("classifier_spring", "spring_female_4_state_model_base.rds"))
saveRDS(results_initial$model, file = here("classifier_spring", "spring_female_4_state_parameters_base.rds"))
```
