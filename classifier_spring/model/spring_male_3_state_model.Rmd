...
Cut out loops produced by the crawl. Use if the haul-out model doesn't work
```{r}
# t <- amwo_hmm %>% 
#   group_by(ID) %>% 
...
#       filter(locType == "o")
#     
#     rows_to_remove <- map(1:(nrow(observed_locs)-1), function(i){
...
#         as.numeric(.)/1000 #results in km
#       
#       total_dist <- x[pull(point_1, rownum):pull(point_2, rownum),] %>% 
#         prepData(type = "LL") %>% 
#         pull(step) %>% 
#         sum(na.rm = TRUE)
# 
# 
#       if(total_dist > (10*point_dist)){ #if the total step distance is more than 10 times the distance between observed points, remove the predicted points from the dataset
#         return((pull(point_1, rownum)+1):(pull(point_2, rownum)-1))
#       } else {
#         numeric()
#       }
#     }) %>%  unlist()
#     # actually remove those rows
#     x <- x[-rows_to_remove,]
#     
#     x$rownum <- NULL
#     
...
fixPar <- list(beta=matrix(c(NA, -100, -100, # pre-migration to transitions, can't go pre- to post
                     ...
                     -100, NA, -100, #Once entered stopover, can only go back to migration
