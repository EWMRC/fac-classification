...
Extract all locations from 2018, 2019, 2020, 2021, and 2022 from Jan 5th through June 30th
```{r include=FALSE}
...
amwo_2019 <- getMovebankData(study = "American Woodcock Migration Ecology in Eastern North America", login = readRDS(here::here("movebank_credentials.rds")), timestamp_start = "20190105000000000", timestamp_end = "20190730000000000", removeDuplicatedTimestamps=TRUE) %>% 
  as.data.frame() %>% 
  filter(timestamp < if_else(local_identifier == "SC-2019-03", ymd_hms("2019-7-30 00:00:00"), ymd_hms("2019-6-30 00:00:00"))) #include an additional month worth of locations, but only for one bird that settles late

amwo_2020 <- getMovebankData(study = "American Woodcock Migration Ecology in Eastern North America", login = readRDS(here::here("movebank_credentials.rds")), timestamp_start = "20200105000000000", timestamp_end = "20200730000000000", removeDuplicatedTimestamps=TRUE) %>% 
  as.data.frame() %>% 
  filter(timestamp < if_else(local_identifier == "VA-2019-36", ymd_hms("2020-7-30 00:00:00"), ymd_hms("2020-6-30 00:00:00"))) #include an additional month worth of locations, but only for one bird that settles late
...
amwo_2022 <- getMovebankData(study = "American Woodcock Migration Ecology in Eastern North America", login = readRDS(here::here("movebank_credentials.rds")), timestamp_start = "20220105000000000", timestamp_end = "20220730000000000", removeDuplicatedTimestamps=TRUE) %>% 
  as.data.frame() %>% 
  filter(timestamp < if_else(local_identifier == "VA-2022-100", ymd_hms("2022-7-30 00:00:00"), ymd_hms("2022-6-30 00:00:00"))) #include an additional month worth of locations, but only for one bird that settles late

amwo_2023 <- getMovebankData(study = "American Woodcock Migration Ecology in Eastern North America", login = readRDS(here::here("movebank_credentials.rds")), timestamp_start = "20230105000000000", timestamp_end = "20230630000000000", removeDuplicatedTimestamps=TRUE) %>% as.data.frame()

amwo_2024 <- getMovebankData(study = "American Woodcock Migration Ecology in Eastern North America", login = readRDS(here::here("movebank_credentials.rds")), timestamp_start = "20240105000000000", timestamp_end = "20240630000000000", removeDuplicatedTimestamps=TRUE) %>% as.data.frame()

amwo_2025 <- getMovebankData(study = "American Woodcock Migration Ecology in Eastern North America", login = readRDS(here::here("movebank_credentials.rds")), timestamp_start = "20250105000000000", timestamp_end = "20250630000000000", removeDuplicatedTimestamps=TRUE) %>% as.data.frame()
```

Formatting and cleaning, including creating time columns and simplifying to one location per day
```{r}
amwo_spring <- as.data.frame(rbind(amwo_2018, amwo_2019, amwo_2020, amwo_2021, amwo_2022, amwo_2023, amwo_2024, amwo_2025))
...
nested_birds_2.5$data <- nested_birds_2 %>% 
  ...
  future_map(.f = function(x){
    ...
    for(i in 1:(nrow(observed_locs)-1)){
      ...
      if(point_dist > 16100 & ((point_2$rownum - point_1$rownum) > 1)){ # there must be predicted points between the observed locations for this to work
...
```{r}
save.image(file=here("classifier_spring", "spring_female_data_temp.RData"))
# load(file=here("classifier_spring", "spring_female_data_temp.RData"))
```
...
#remove early spring movements which throw off the hmm for NJ-2018-08-2019, NJ-2018-13-2019, and NJ-2018-15-2019.
```{r}
amwo_hmm <- amwo_hmm %>%
...
amwo_hmm <- amwo_hmm %>% #removing ranging movements
  #filter(!(ID == "AL-2022-15-2022" & time == ymd_hms("2022-02-28 17:00:00"))) %>% #This shouldn't throw off the designation in the new model. Exclude from paper, maybe remove later.
  #filter(!(ID == "AL-2021-12-2021" & time == ymd_hms("2021-02-25 17:00:00"))) %>% #This shouldn't throw off the designation in the new model. Exclude from paper, maybe remove later.
  #filter(!(ID == "NJ-2018-15-2019" & time == ymd_hms("2019-01-14 04:00:00"))) %>% # duplicate of exclusion code from above, uses a non-existent location. Exclude from paper, maybe remove later.
  #filter(!(ID == "VA-2021-98-2022" & time > ymd_hms("2022-05-30 15:00:00"))) %>% #This shouldn't throw off the designation in the new model. Exclude from paper, maybe remove later.
  #filter(!(ID == "VA-2022-100-2022" & time > ymd_hms("2022-06-19 09:00:00"))) %>% #This shouldn't throw off the designation in the new model. Exclude from paper, maybe remove later.
  #filter(!(ID == "AL-2021-10-2021" & time == ymd_hms("2021-05-21 03:58:56"))) %>% #This shouldn't throw off the designation in the new model. Exclude from paper, maybe remove later.
  #filter(!(ID == "AL-2021-10-2021" & time == ymd_hms("2021-05-21 17:00:00"))) %>% #This shouldn't throw off the designation in the new model. Exclude from paper, maybe remove later.
  #filter(!(ID == "AL-2021-12-2021" & time == ymd_hms("2021-02-26 05:02:56"))) %>% #This shouldn't throw off the designation in the new model. Exclude from paper, maybe remove later.
  filter(!(ID == "ME-2018-13-2019" & julian_day == 7)) %>% #late fall migration
  filter(!(ID == "RI-2021-46-2022" & julian_day %in% c(41, 43, 45))) %>% 
  filter(!(ID == "VA-2020-66-2021" & julian_day %in% c(24, 29))) %>%  #foray loop
  filter(!(ID == "NY-2018-07-2019" & julian_day %in% c(28:35))) %>% #foray loop
  filter(!(ID == "RI-2020-44-2021" & julian_day %in% c(29:43)))
```
