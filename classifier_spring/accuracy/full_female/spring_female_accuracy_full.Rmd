...
Par0_m1 <- list(step=c(0.215581430, 2.327105e+02, 0.765139476, 0.165041155, #mean in km
                ...
                y = c(35.100240, 40.609473, 41.336072, 44.704289,
                      2.559318, 4.513901, 3.783183, 3.066589),
                julian_day = c(41.72752, 86.74204, 91.90514, 140.49721, #mean
                               20.45794, 25.55749, 26.53405, 24.61851),#concentration
                log_mean_dist_7 = c(3.548430, 8.795949, 4.772995, 3.147517, #mean
                                    1.573726, 3.681093, 3.031189, 1.616383), #sd
                residence_time = c(49.2763, 12.36830, 25.15765, 65.96781, #mean
                                   21.4730, 19.72401, 19.80745, 23.04604), #sd
                dist_start = c(0.02081405, 0.830695, 0.977581, 0.9999991), #prob
                step_500 = c(1e-09, 0.99999, 1e-09, 1e-09), #prob
                breeding_abundance = c(0.1174643, 0.4356124, 0.5685293, 0.8016264)) #prob
```
...
prior_results <- read.csv("accuracy_results_female_full.csv") %>% #ensure that I'm not duplicating past labor
  transmute(combined = paste0(cutoff_date, "_",ID))

#How many more samples do I need?
read.csv("accuracy_results_female_full.csv") %>% 
  pull(ID) %>% 
  unique() %>% 
  length()
...
write.table(accuracy_results_female, "accuracy_results_female_full.csv",
            append = TRUE,
            sep = ",",
            col.names = FALSE,
            row.names = FALSE,
...
```{r}
accuracy_results_female <- read_csv("accuracy_results_female_full.csv")
```
